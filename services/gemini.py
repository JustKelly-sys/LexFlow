...
def build_prompt(hourly_rate: int, policy_context: str | None = None) -> str:
    """Build the extraction prompt with the user's hourly rate and optional RAG context.

    policy_context: retrieved billing policy chunks as a single pre-formatted string.
    If None, prompt is identical to the original (backward compatible).
    """
    context_block = ""
    if policy_context:
        context_block = (
            f"\n\nBILLING POLICY CONTEXT\n"
            f"The following billing policies apply to this matter. "
            f"Apply them when calculating billable amounts and flagging special conditions:\n\n"
            f"{policy_context}\n\n"
            f"--- END OF POLICY CONTEXT ---\n"
        )
    ...
    return (
        ...
        f"{context_block}\n\n"
        f"IMPORTANT: The voice note may describe work for MULTIPLE clients or matters. "
        f"If so, create a SEPARATE entry for each distinct client/matter.\n\n"
        ...
        f"- billable_amount: Calculated as duration x hourly rate of R{hourly_rate}/hr, "
        f"formatted as 'RXXXX' (Rands only, no cents). "
        f"Example: 2 hours = 'R{hourly_rate * 2}', 30 minutes = 'R{hourly_rate // 2}'. "
        f"If policy context indicates a rate modifier (travel = 50%, after-hours = 150%), apply it.\n\n"
