...
def _make_httpx_mock(**method_results):
    ...
    # Patch the httpx module attribute itself — all routers/services share it
    return patch("httpx.AsyncClient", mock_constructor)
...
class TestCORSConfig:
    ...
    def test_allowed_origin_app_url(self, client):
        """CORS should allow requests from the configured public APP_URL."""
        from config import APP_URL
        r = client.options("/billing", headers={
            "Origin": APP_URL,
            "Access-Control-Request-Method": "POST",
        })
        assert r.headers.get("access-control-allow-origin") == APP_URL
